...
class Downloader:
    ...
    def __call__(self, link: Link, location: str) -> tuple[str, str]:
        ...
        import re
        try:
            logger.info("Downloading with aria2c: %s", link.url_without_fragment)
            ...
            renderer = get_download_progress_renderer(
                bar_type=self._progress_bar, size=download_size, initial_progress=0
            )

            def aria2_progress_generator():
                buffer = ""
                last_bytes = 0
                while True:
                    ...
                    buffer += char
                    if char in ('\r', '\n'):
                        match = re.search(r'\[#\w+\s+(\d+)B/', buffer)
                        if match:
                            current_bytes = int(match.group(1))
                            if current_bytes > last_bytes:
                                yield b"0" * (current_bytes - last_bytes)
                                last_bytes = current_bytes
                        buffer = ""
                        
                if download_size and download_size > last_bytes:
                    yield b"0" * (download_size - last_bytes)

            for _ in renderer(aria2_progress_generator()):
                pass
            ...
            logger.warning("aria2c failed: %s. Falling back to requests.", e)
